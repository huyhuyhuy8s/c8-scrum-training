...
export const approveRequestController = async (req, res) => {
    try {
        ...
        const updatedRequest = await approveRequest(id, managerId, comment || "Approved");
...
export const rejectRequestController = async (req, res) => {
    try {
        ...
        const updatedRequest = await rejectRequest(id, managerId, comment ? comment.trim() : "Rejected");
...
export const updateExpenseRequestController = async (req, res) => {
    try {
        ...
        if (!description && !amount && !image) {
            return res.status(400).json({
                success: false,
                message: "At least one field (description, amount, or image) must be provided for update"
            });
        }
