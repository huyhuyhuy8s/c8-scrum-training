...
export const createExpense = async (expenseData) => {
  try {
    ...
    if (error.response) {
      ...
      switch (status) {
        case 400:
          throw new Error(data?.message || 'Invalid request data. Please check your input.');
        case 401:
          throw new Error('Authentication required. Please log in.');
        case 403:
          throw new Error('You do not have permission to create expense requests.');
        case 422:
...
export const getAllExpenses = async () => {
  try {
    const response = await axios.get(`${API_BASE_URL}/employee/${DEFAULT_USER_ID}`, {
      timeout: 10000,
    });
...
export const updateExpense = async (expenseId, expenseData) => {
  try {
    ...
    if (expenseData.image !== undefined) payload.image = expenseData.image ? 'image_placeholder' : null;
